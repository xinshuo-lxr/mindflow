...
package com.xinshuo.mindflow.rag.core.intent;

import cn.hutool.core.collection.CollUtil;
import com.xinshuo.mindflow.rag.dto.IntentCandidate;
import com.xinshuo.mindflow.rag.dto.IntentGroup;
import com.xinshuo.mindflow.rag.dto.SubQuestionIntent;
...
import static com.xinshuo.mindflow.rag.constant.RAGConstant.INTENT_MIN_SCORE;
import static com.xinshuo.mindflow.rag.constant.RAGConstant.MAX_INTENT_COUNT;
import static com.xinshuo.mindflow.rag.enums.IntentKind.SYSTEM;

/**
 * 意图解析器
 * <p>
 * 负责对用户问题进行意图分类，支持单问题识别和（未来）多子问题并行识别。
 * Step 11 提供单问题 resolve，Step 12 加入 RewriteResult 多子问题支持。
 */
@Slf4j
@Service
public class IntentResolver {
    ...
    /**
     * 对单个问题进行意图分类（Step 11 使用）
     *
     * @param question 用户问题
     * @return 过滤后的 NodeScore 列表（score >= INTENT_MIN_SCORE，最多 MAX_INTENT_COUNT 个）
     */
    public List<NodeScore> resolve(String question) {
        return classifyIntents(question);
    }

    /**
     * 对多子问题进行并行意图分类（Step 12 接入 RewriteResult 后使用）
     *
     * @param subQuestions 子问题列表
     * @return 各子问题的意图分类结果
     */
    public List<SubQuestionIntent> resolve(List<String> subQuestions) {
    ...
    /**
     * 合并 MCP 和 KB 意图分组（单问题版本）
     */
    public IntentGroup mergeIntentGroup(List<NodeScore> scores, boolean dummy) {
        List<NodeScore> mcpIntents = new ArrayList<>(NodeScoreFilters.mcp(scores));
        List<NodeScore> kbIntents = new ArrayList<>(NodeScoreFilters.kb(scores));
        return new IntentGroup(mcpIntents, kbIntents);
    }
